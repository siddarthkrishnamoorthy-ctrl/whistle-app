...
export default function ChessArenaScreen() {
  ...
  async function play(opponentClientId: string) {
    ...
    try {
      ...
      if (res.game?.id) router.push(`/parent/chess/${res.game.id}`);
  ...
  async function playBot(level: number) {
    ...
    try {
      ...
      router.push(`/parent/chess/${game.id}`);
    } catch (e) {
      Alert.alert("Couldn't start", e instanceof Error ? e.message : "Please try again.");
    } finally {
      setBusy(false);
  ...
  async function respond(challengeId: string, accept: boolean) {
    try {
      ...
      if (accept && res.game?.id) router.push(`/parent/chess/${res.game.id}`);
      load();
    } catch (e) {
      ...
      g.status === "active"
        ...
        : g.winner === "draw"
          ...
          : (g.winner === "white") === iAmWhite
            ...
            : "They won";
    return (
      <TouchableOpacity key={g.id} onPress={() => router.push(`/parent/chess/${g.id}`)} activeOpacity={0.75}>
    ...
    <ScrollView contentContainerStyle={{ padding: 20, gap: 14 }}>
      ...
      <View>
        ...
        <Card>
          ...
          <TouchableOpacity
            onPress={() => router.push("/parent/chess/puzzle")}
