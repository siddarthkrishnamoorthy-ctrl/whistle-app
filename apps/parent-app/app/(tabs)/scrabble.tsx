...
export default function ScrabbleArenaScreen() {
  ...
  async function play(opponentClientId: string) {
    ...
    try {
      ...
      if (res.game?.id) router.push(`/scrabble/${res.game.id}`);
  ...
  async function respond(challengeId: string, accept: boolean) {
    try {
      ...
      if (accept && res.game?.id) router.push(`/scrabble/${res.game.id}`);
      load();
    } catch (e) {
  ...
  const gameRow = (g: Game) => {
    ...
    return (
      <TouchableOpacity key={g.id} onPress={() => router.push(`/scrabble/${g.id}`)} activeOpacity={0.75}>
