...
function promptBlankLetter(): string {
  if (Platform.OS === "web" && typeof window !== "undefined") {
    const v = window.prompt("Blank tile — which letter should it be? (a–z)", "e");
    const l = (v ?? "e").trim().toLowerCase()[0];
    return /[a-z]/.test(l) ? l : "e";
  }
  return "e";
}
...
export default function ScrabbleBoard({
  ...
  function tapCell(index: number) {
    ...
    const blank = tile === "?";
    const letter = blank ? promptBlankLetter() : tile;
    onChange([...pending, { index, letter, rackIdx: selRack, blank }]);
    setSelRack(null);
  }
